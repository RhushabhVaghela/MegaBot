...
Extracted from the monolithic ``orchestrator.py`` to separate HTTP
concerns from orchestration logic.  The canonical ``orchestrator`` variable
lives in ``megabot.core.orchestrator`` — route handlers access it lazily via
``_get_orchestrator()`` so that tests which set
``orch_module.orchestrator = mock`` are honoured automatically.
"""
...
def _get_orchestrator() -> Any:
    ...
    """
    orch_mod = sys.modules.get("megabot.core.orchestrator")
